...
module ApplicationHelper
    ...
    def can_view?(user, event, roles, &block)
    ...
    def can_view_or_is_owner?(user, event, item, roles, &block)
        if logged_in? && (user.is_in_role?(event, roles) || item.user_id == user.id)
            content = capture(&block)
            concat(content, block.binding)
        end
    end
    ...
    def restrict_access(user, event, roles)
        redirect_to "/not_authorized" unless logged_in? && user.is_in_role?(event, roles)
    end
